...
const DatabaseManager = () => {
    ...
    const deleteRecord = (record: any) => {
        ...
        Modal.confirm({
            ...
            onOk: async () => {
                ...
                const index = tableDatas.findIndex(
                    (item) => item[KeyField] === KeyFieldVal
                );
    ...
    const save = async (key: any) => {
        try {
            ...
            const index = newData.findIndex(
                (item) => item[KeyField] === form.getFieldValue(KeyField)
            );
            if (index > -1) {
    ...
    useEffect(() => {
        if (typeof selectedTable == "string") {
            getTableDetail(selectedTable)
                .then((res) => {
                    const findKey = (res.data as Array<any>).find(
                        (item) => item.Key == "PRI"
                    );
                    console.log(findKey, findKey.Field);
                    SetKeyField(findKey.Field);

                    const keys = Object.keys(lodash.keyBy(res.data, "Field")).map(
                        (item, index) => ({
                            title: item.charAt(0).toUpperCase() + item.slice(1), // 首字母大写
                            dataIndex: item,
                            key: index,
        ...
        <Layout style={{ height: "100vh" }}>
            ...
            >
                <Menu mode="inline" defaultSelectedKeys={["1"]}>
                    <Menu.Item>
                        <Button type={"link"} onClick={() => setViewMode("CREATETABLE")}>
                            {" "}
                            Create Table{" "}
                        </Button>
                    </Menu.Item>
                    ...
                    {viewMode != "CREATETABLE" && (
                        <SpaceBetween>
                            <div>
                                ...
                                <Button
                                    type={viewMode === "STRUCT" ? "primary" : "default"}
                ...
                <Menu>
                    {database.map((item, index) => {
                        return (
                            <Menu.Item
                                key={item + index}
                                onClick={() => handleResetDabase(item)}
                            >
                                {" "}
                                {item}
                            </Menu.Item>
            ...
            />
            <QuerySqlModal
                visible={isQuerySqlVisible}
                onCancel={() => SetQuerySqlVisible(false)}
                source={DBSource}
                database={currentDB}
            ></QuerySqlModal>
        </Layout>
